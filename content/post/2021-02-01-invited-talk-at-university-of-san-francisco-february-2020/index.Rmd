...
```{r pkg load, message=FALSE, ECHO = TRUE, collapse= TRUE}
...
#library(formatR) # help with tidying the code chunks
```
...
```{r hsgrad1, message=FALSE, ECHO = TRUE, collapse= TRUE, eval= FALSE}
...
```{r hsgrad2, message=FALSE, ECHO = TRUE, collapse= TRUE, eval= FALSE}
...
The projected HS grad stats didn't have values for UC/CSU grads, so I needed to impute that as part of the merging of the actual & projected files. I also calculated year-over-year percent changes for a few fields. (though did it manually not with a function like in the [Tidy Tuesday HBCU post](https://www.gregdubrow.io/2021/02/25/tidy-tuesday-february-2-20201-hbcu-enrollment/)). The code below gets us the `cahsgrads_1993_2028` dataframe.
```{r hsgrad3, message=FALSE, ECHO = TRUE, collapse= TRUE, eval= FALSE}
...
Now that we have the data, let's make the chart I presented to the group, showing actual & projected high school grads in California, breaking out the [UC/CSU eligible](https://www.cde.ca.gov/ci/gs/hs/hsgrtable.asp) grads. It's fairly straightforward ggplot code, so no need to display here.
...
```{r dataviz1, fig.width=8, fig.height=4.0, dpi = 300,  echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
cahsgrads_1993_2028 %>%
...
```{r ipedsids, message=FALSE, ECHO = TRUE, collapse= TRUE, error= FALSE}
...
First up is USF enrollment from Fall 1987 to Fall 2018 (the latest year that IPEDS had available as of February 2020). The ggplot code is mostly basic, so no need to show all of it here. What I will show is [a neat solution](https://stackoverflow.com/a/52920047/10226848) to a crowded x axis - the `every_nth` function to count every n value, and apply it to the breaks - in this case I set it to n=3. I'd tried `scales::pretty_breaks()` but it didn't work. I also used `ggrepel` to move the labels a bit.

Not shown here, but I used [`patchwork`](https://patchwork.data-imaginist.com/) to stitch together the undergrad and grad enrollment plots. 

```{r dataviz2, fig.width=8, fig.height=5.0, dpi = 300, echo = TRUE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz3, fig.width=8, fig.height=5.0, dpi = 300, echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
plot_usfenr_gr <-
	ipeds_fallenroll_8718 %>%
...
```{r jescoll1, fig.width=8, fig.height=5.0, dpi = 300, echo = TRUE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz4, echo = TRUE, evaluate = FALSE, fig.show='hide'}
