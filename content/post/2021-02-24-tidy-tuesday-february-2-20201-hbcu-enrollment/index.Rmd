---
title: Tidy Tuesday, February 2, 20201 - HBCU Enrollment
author: Greg Dubrow
date: '2021-02-25'
slug: tidy-tuesday-february-2-20201-hbcu-enrollment
categories:
  - post
tags: 
  - R
...
So let's dive into the data. First we'll load the sets from TidyTuesday and then the Delta Cost Project files.

_(btw, if anyone has tips for how to do code-folding / show-hide in the latest version of Hugo Academic, let me know...would love to include that as a viewing option.)_

```{r pkgload, message=FALSE, ECHO = TRUE}
# load packages
...
```{r dataload2, message=FALSE, ECHO = FALSE, warning = FALSE, include = FALSE}
...
```{r dataload3, message=FALSE, ECHO = FALSE, results = 'hide'}
...
```{r dataload5, message=FALSE, ECHO = FALSE, results = 'hide', include = FALSE}
...
```{r dataload6, message=FALSE, ECHO = FALSE, results = 'hide'}
...
```{r dataload7, message=FALSE, ECHO = FALSE, results = 'hide', echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
...
Ok, the data is in place, let's see what it tells us! I'll hide the code for the more basic charts. Again, the full code file it in [my Tidy Tuesday repo](https://github.com/greg-dubrow/tidy-tuesday-projects/tree/master/2021/tt_2021-02-02_hbcu). You'll note that this analysis is mostly line graphs. That'a because I'm mostly doing trend analysis, and I like line graphs for time series. 

First up we'll use the Tidy Tuesday data to look at HBCU enrollment by ethnicity (Black and non-Black) from 1990 to 2015. What do we see? Overall, periods of increase and decrease in total enrollment. But interestingly we see that non-Black enrollment kept increasing, even as Black enrollment started to drop. Of course Black student make up a vast majority of HBCU enrollment, so their trends drive overall numbers. It is worth noting though that the changes started in the 2010s, coinciding with [changes in how NCES counted ethncity](https://nces.ed.gov/ipeds/report-your-data/race-ethnicity-reporting-changes). It's beyond the scope of this quick (ha) analysis, but worth digging deeper to see the effect of changes on the ethnic mix of HBCUs.

```{r dataviz1, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz2, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz3, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = TRUE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz4, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
...
So we've done what we can with the Tidy Tuesday data, time to compare HBCUs to non-HBCUs using the Delta Cost Project data. We'll stay with sector analysis and look at enrollments by sector. To make it look like I wanted, I used Thomas Lin Pederson's [`patchwork` package](https://patchwork.data-imaginist.com/). I'll suppress code that creates the component charts and just show the patchwork call.

```{r dataviz5, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
enr19902015hbcu <-
tuitenr_8715_agg %>%
  filter(hbcu == 1) %>% 
...
```{r dataviz6, fig.width=7.5, fig.height=6.0, dpi = 300,  echo = TRUE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz7, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = TRUE, warning=FALSE, message = FALSE, error= FALSE}
tuitsect_hbcu <-
	tuitenr_8715_agg %>%
...
```{r dataviz8, fig.width=7.5, fig.height=6.0, dpi = 300,  echo = TRUE, warning=FALSE, message = FALSE, error= FALSE}
...
```{r dataviz9, fig.width=7.5, fig.height=4.0, dpi = 300,  echo = FALSE, warning=FALSE, message = FALSE, error= FALSE}
tuit_8715_pctchgh %>%
